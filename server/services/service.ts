...
export default ({ strapi }: { strapi: Strapi }) => ({
  ...
  encrypt(value: string): string {
    ...
    const iv = stringToIV(process.env.IV)
...
const stringToIV = (inputString:string):string => {
  const hash = crypto_1.createHash('sha256').update(inputString).digest();
  return hash.slice(0, 16);  // Take the first 16 bytes for a 128-bit IV
}